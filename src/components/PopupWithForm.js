export default function PopupWithForm(props) {
  return (
    <div className={`popup popup_type_${props.name} ${props.isOpen ? 'popup_opened' : ''}`} onClick = {props.onOvelayClick}>
      <div className="popup__container" onClick = {(e) => e.stopPropagation()}>
        ...
        <form className={`popup__form popup__form_type_${props.name}`} method="POST" name={props.name} onSubmit={props.onSubmit}>
          {props.children}
          <button type="submit" className="popup__submit-button" aria-label="Отправить данные">{props.buttonText}</button>
        </form>
      </div>
    </div>
  )
